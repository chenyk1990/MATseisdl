function [dout]=dl_ddtf_recon(d0,mask,mode,l,s,perc,niter,a,param)
%dl_ddtf_recon: DDTF algorithm for 2D/3D reconstruction
% BY Yangkang Chen
% Jan, 2020
%
% INPUT
...
%
% DEMO:
% test/test_dl_sgk_recon.m
% test/test_dl_sgk_recon3d.m
...
for iter=1:niter
    if n3==1
        X=dl_patch(d1,mode,l1,l2,s1,s2);
        [Dsgk,Gsgk]=dl_ddtf(X,param);
        Gsgkc=Gsgk;
        Gsgk=dl_pthresh(Gsgkc,'ph',perc);
        X2=Dsgk*Gsgk;
        d2=dl_patch_inv(X2,mode,n1,n2,l1,l2,s1,s2);
    else
        X=dl_patch3d(d1,mode,l1,l2,l3,s1,s2,s3);
        [Dsgk,Gsgk]=dl_ddtf(X,param);
        Gsgkc=Gsgk;
        Gsgk=dl_pthresh(Gsgkc,'ph',perc);
        X2=Dsgk*Gsgk;
        d2=dl_patch3d_inv(X2,mode,n1,n2,n3,l1,l2,l3,s1,s2,s3);
    end
