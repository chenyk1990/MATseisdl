...
tic
[d1,D,G,D0]=dl_sgk_denoise(d,mode,[l1,l2,l3],[s1,s2,s3],perc,param);
toc
dl_snr(dc,d1,2) %12.89 dB

figure;imagesc([dc(:,:,10),d(:,:,10),d1(:,:,10),d(:,:,10)-d1(:,:,10)]);

%% benchmark with KSVD
tic
[d2,D2,G2,D02]=dl_ksvd_denoise(d,mode,[l1,l2,l3],[s1,s2,s3],perc,param);
toc
figure;imagesc([dc(:,:,10),d(:,:,10),d2(:,:,10),d(:,:,10)-d2(:,:,10)]);

%% SNR
dl_snr(dc,d,2)  %-0.01 dB
dl_snr(dc,d1,2) %12.89 dB
dl_snr(dc,d2,2) %13.33 dB
